...
Internet / Browser
    │
    ▼
nginx (:3001)  ←── Einstiegspunkt für allen Traffic
    │
    ├── /api/v1/*  ─────────► FastAPI (:8000)
    │                               │
    │                    SQLite DB + GPU-Semaphore
...
MLflow (:3004)  ←── von dev-start.sh gestartet
\`\`\`

## nginx

CTIP liefert eine User-Space nginx-Konfiguration unter \`nginx-local/nginx.conf\`.
Kein Root erforderlich — läuft als normaler Benutzer.
...
### Öffentliche Domain konfigurieren

1. In \`.env\`: \`PUBLIC_DOMAIN=mylab.ddns.net\`
2. In nginx.conf: \`server_name\` auf Domain setzen
3. SSL mit Let's Encrypt: \`sudo certbot --nginx -d mylab.ddns.net\`
4. Router: Port 3001 weiterleiten

## FastAPI Backend (:8000)

Startup-Sequenz:
1. Verzeichnisse erstellen (DATA_ROOT, MODELS_DIR, OUTPUTS_DIR)
2. Datenbank initialisieren (SQLite via SQLModel)
3. GPU-Broadcast-Loop starten
4. \`asyncio.Semaphore(1)\` erstellen (GPU Guard)

**GPU Guard**: Nur ein GPU-Task gleichzeitig. Alle weiteren Anfragen warten in der Warteschlange. Verhindert OOM-Crashes auf der RTX 4060.
...
| mlflow | 3004 | MLflow |
| label-studio | 3005 | Label Studio |
| cvat | 3006 | CVAT |

## Datenbank (SQLite)

\`trichome.db\` im Repository-Root. Für Produktion mit mehreren Nutzern:
\`\`\`bash
DATABASE_URL="postgresql://user:pass@localhost:5432/ctip"
\`\`\`
...
\`\`\`bash
./scripts/dev-start.sh           # alles starten
./scripts/dev-start.sh stop      # alles stoppen
./scripts/dev-start.sh status    # Status anzeigen
...
\`\`\`
Navegador → nginx (:3001) → FastAPI (:8000) + Next.js (:3000)
Docker: Label Studio (:3005), CVAT (:3006), MLflow (:3004)
\`\`\`

## nginx (desarrollo local)

Configuración en \`nginx-local/nginx.conf\` — se ejecuta como usuario normal, sin root.
...
Un semáforo asyncio (\`Semaphore(1)\`) garantiza que solo una tarea GPU se ejecute a la vez — previene crashes OOM en la RTX 4060.
...
\`\`\`bash
./scripts/dev-start.sh          # iniciar todo
./scripts/dev-start.sh stop     # detener todo
./scripts/dev-start.sh status   # mostrar estado
tail -f logs/backend.log        # ver logs
\`\`\`
`;

const page: WikiPage = {
  slug: 'infrastructure',
  title: { en: 'Infrastructure', de: 'Infrastruktur', es: 'Infraestructura' },
  description: {
    en: 'nginx, Docker, FastAPI, SQLite, MLflow — how all services connect.',
    de: 'nginx, Docker, FastAPI, SQLite, MLflow — wie alle Services zusammenhängen.',
    es: 'nginx, Docker, FastAPI, SQLite, MLflow — cómo se conectan todos los servicios.',
  },
  content: { en, de, es },
  section: 'setup',
  icon: '🏗️',
